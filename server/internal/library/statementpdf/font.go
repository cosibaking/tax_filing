...
// ChineseFontDataForReport loads the repository's Chinese font for monthly reports.
// Callers must treat the returned bytes as read-only.
func ChineseFontDataForReport() ([]byte, error) {
	paths := []string{
	...
	return nil, gerror.New("未找到月度报告中文字体 resource/captcha/fonts/SourceHanSansCN-Normal.ttf")
}
